...
const COLOR_ESTADO = {
    'En ejecución':                              '#16a34a',
    'En ejecución (Modificado)':                 '#15803d',
    'Ampliado':                                  '#0ea5e9',
    'Terminado':                                 '#6b7280',
    'Terminado anticipadamente por mutuo acuerdo': '#f59e0b',
    'Terminado anticipadamente por sanción':      '#ef4444',
};
...
// ─── Barra horizontal para gráficos ───────────────────────────────────────────

function BarChart({ datos, labelKey, valueKey, color = '#7c3aed', fmt: fmtVal = fmtN }) {
        ...
        <div>
            <div className="kpi-grid" style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: 16, marginBottom: 28 }}>
                <KpiCard
                    ...
                    valor={fmtN(stats.total)}
                    icono="📋"
                    color="#7c3aed"
                />
                <KpiCard
                    titulo="Vigentes"
                    valor={fmtN(stats.vigentes)}
                    subtitulo="En ejecución / Ampliado"
                    icono="✅"
                    color="#16a34a"
                />
                <KpiCard
                    titulo="Terminados"
                    valor={fmtN(stats.terminados)}
                    subtitulo={`${pct(stats.terminados / stats.total * 100)} del total`}
                    icono="🏁"
                    color="#6b7280"
                />
                <KpiCard
                    titulo="Monto Total"
                    valor={fmt(stats.monto_total)}
                    icono="💰"
                    color="#0ea5e9"
                />
                <KpiCard
                    titulo="Monto Ejecutado"
                    valor={fmt(stats.monto_ejecutado)}
                    subtitulo={`${pct(stats.pct_ejecutado)} ejecutado`}
                    icono="📊"
                    color="#f59e0b"
                />
                <KpiCard
                    titulo="Con Garantías por Vencer"
                    valor={fmtN(stats.con_garantias_por_vencer)}
                    icono="⚠️"
                    color={stats.con_garantias_por_vencer > 0 ? '#f59e0b' : '#6b7280'}
                />
                <KpiCard
                    titulo="Con Garantías Vencidas"
                    valor={fmtN(stats.con_garantias_vencidas)}
                    icono="🔴"
                    color={stats.con_garantias_vencidas > 0 ? '#dc2626' : '#6b7280'}
                />
                <KpiCard
                    titulo="Con Sanciones"
                    valor={fmtN(stats.con_sanciones)}
                    icono="⚡"
                    color={stats.con_sanciones > 0 ? '#ef4444' : '#6b7280'}
                />
            </div>
    ...
    return (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
            <div className="card">
                ...
                <BarChart
                    datos={stats.por_estado}
                    labelKey="estado_contrato"
                    valueKey="total"
                    color="#7c3aed"
                />
            </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
                <div className="card">
                    ...
                    <BarChart
                        datos={stats.por_categoria}
                        labelKey="categoria_contrato"
                        valueKey="total"
                        color="#16a34a"
                    />
                </div>
                <div className="card">
                    ...
                    <BarChart
                        datos={stats.por_ejecucion}
                        labelKey="ejecucion_contrato"
                        valueKey="total"
                        color="#f59e0b"
                    />
                </div>
            ...
            <div className="card">
                <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 16, color: '#374151' }}>
                    Por Tipo de Contrato (Top 15)
                </div>
                <BarChart
                    datos={stats.por_tipo?.slice(0, 15)}
                    labelKey="tipo_contrato"
                    valueKey="total"
                    color="#7c3aed"
                />
            </div>
            <div className="card">
                <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 16, color: '#374151' }}>
                    Por Unidad Requirente
                </div>
                <BarChart
                    datos={stats.por_unidad}
                    labelKey="unidad_requirente"
                    valueKey="total"
                    color="#0ea5e9"
                />
            </div>
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <div className="kpi-grid" style={{ gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }}>
                ...
                />
                <KpiCard
                    titulo="Con Sanciones Solicitadas"
                    valor={fmtN(stats.con_sanciones)}
                    subtitulo="contratos afectados"
                    icono="⚡"
                    color={stats.con_sanciones > 0 ? '#ef4444' : '#6b7280'}
            ...
            <div className="card" style={{ background: '#fffbeb', border: '1px solid #fcd34d' }}>
                <div style={{ fontSize: 12, color: '#92400e' }}>
                    <strong>Nota:</strong> Los datos de garantías y sanciones provienen directamente
                    del archivo Excel de Mercado Público. Actualiza el archivo periódicamente para
...
export function GestionContratosPage() {
    ...
    const handleCancelar = async () => {
        ...
        try {
            await cancelarActualizacionContratos(tarea.task_id);
        } catch { /* ignorar */ }
        clearInterval(pollingRef.current);
            ...
            <div className="page-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                ...
                <button
                    ...
                    style={{
                        ...
                        fontSize: 13, fontWeight: 600, cursor: enProceso ? 'not-allowed' : 'pointer',
            ...
            {stats?.vacio && !enProceso && (
                <div style={{
                    ...
                    <div>
                        ...
                        <em>Mercado Público → Contratos → SSO</em>{' '}
                        usando el script <code>descargar_contratos_sso_selenium.py</code>,
