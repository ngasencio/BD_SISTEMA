...
export default function ModalActualizarFacturas({ onConfirmar, onCerrar }) {
    const [usuario, setUsuario] = useState('');
    const [password, setPassword] = useState('');
    const [fechaDesde, setFechaDesde] = useState(hoyISO(-7));
    ...
    return (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 1100, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <div style={{ background: '#fff', borderRadius: 12, padding: 24, width: 400, boxShadow: '0 12px 40px rgba(0,0,0,0.3)' }}>
                <h3 style={{ margin: '0 0 6px' }}>Actualizar Dipres</h3>
                <p style={{ fontSize: 12, color: '#6b7280', margin: '0 0 16px' }}>
                    Descarga las facturas del portal DIPRES/Acepta para el rango de fechas indicado y
                    actualiza la base de datos (los registros existentes se actualizan por folio+emisor,
                    nunca se borra el histórico). Las credenciales no se almacenan en el servidor. Puede
                    tardar varios minutos — puedes cancelarlo en cualquier momento.
                </p>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                    <input
                        placeholder="Usuario DIPRES/Acepta"
                        value={usuario}
                        onChange={e => setUsuario(e.target.value)}
                        style={{ padding: '8px 10px', border: '1px solid #d1d5db', borderRadius: 6 }}
                    />
                    <input
                        placeholder="Contraseña DIPRES/Acepta"
                        type="password"
                        value={password}
                    ...
                    <label style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, color: '#374151' }}>
                        ...
                        👁 Ver navegador (solo mantención)
                    </label>
                    {visible && (
                        <div style={{ fontSize: 11, color: '#92400e', background: '#fffbeb', border: '1px solid #fde68a', borderRadius: 6, padding: '6px 8px' }}>
                            Se abrirá una ventana de Chromium en el escritorio del servidor — <strong>no se transmite
                            a este navegador</strong>. Solo sirve si tienes acceso directo a esa máquina en este
                            momento (necesario para resolver el reCAPTCHA cuando la sesión guardada expiró).
                            Para una actualización normal, deja esta casilla sin marcar.
                        </div>
                    )}
                </div>
