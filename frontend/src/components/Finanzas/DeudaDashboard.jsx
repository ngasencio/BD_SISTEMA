...
/**
 * Normaliza los datos que provienen de la API
 */
...
export default function DeudaDashboard() {
    ...
    const kpis = useMemo(() => {
        ...
        filtered.forEach(r => {
            byProv[r.prov] = (byProv[r.prov] || 0) + r.d;
            byUE[r.ue] = (byUE[r.ue] || 0) + r.d;
        });

        const topProvEntry = Object.entries(byProv).sort((a, b) => b[1] - a[1])[0];
        const topUEEntry = Object.entries(byUE).sort((a, b) => b[1] - a[1])[0];

        return { totalDeuda, totalPagada, totalVigente, pct, topProvEntry, topUEEntry, byProv, byUE };
    }, [filtered]);

    // ── Alertas (Usado en el TabBar para el badge) ──────────────────────────────
    const alertasData = useMemo(() => {
        return Object.entries(kpis.byProv)
            .filter(([, v]) => v > thresholds.medio)
            .map(([prov, deuda]) => {
                let nivel = deuda >= thresholds.critico ? 'critica' : deuda >= thresholds.alto ? 'alta' : 'media';
                return { prov, deuda, nivel };
            })
            .sort((a, b) => b.deuda - a.deuda);
    }, [kpis.byProv, thresholds]);
    ...
    return (
        <div>
            ...
            <div className="content">
                ...
                <FilterZone
                    filters={{ filterUE, filterN1, filterDeuda, filterOrigen }}
                    setters={{ setFilterUE, setFilterN1, setFilterDeuda, setFilterOrigen }}
                    options={{ n1Options }}
                />

                <TabsBar activeTab={activeTab} setActiveTab={setActiveTab} alertasCount={alertasData.length} />

                {activeTab === 'kpis' && <TabKPIs kpis={kpis} filtered={filtered} alertasData={alertasData} />}
                {activeTab === 'alertas' && <TabAlertas alertasData={alertasData} kpis={kpis} onOpenModal={() => setShowModal(true)} />}
                {activeTab === 'detalle' && <TabDetalle filtered={filtered} tiposDocUnicos={tiposDocUnicos} />}
                {activeTab === 'antiguedad' && <TabAntiguedad filtered={filtered} />}
                {activeTab === 'concepto' && <TabConcepto filtered={filtered} kpis={kpis} />}
            </div>
...
// ═══════════════════════════════════════════════════════════════════════════════
// SECCIONES DE TABS (Clean Code: Componentes Funcionales Separados)
// ═══════════════════════════════════════════════════════════════════════════════

function TabKPIs({ kpis, filtered, alertasData }) {
    ...
    const alertasCriticas = alertasData.filter(a => a.nivel === 'critica').length;
    ...
    return (
        <div>
            ...
            <div className="kpi-grid">
                ...
                <KPICard color="celeste" icon="🏥" label="Establec. Mayor Deuda"
                ...
                <KPICard color="rojo" icon="🔴" label="Alertas Críticas Activas" value={alertasCriticas} meta="Según umbrales" />
            </div>
            ...
            <div className="card">
                ...
                <div className="card-body">
                    {topProveedores.map(([prov, deuda], i) => (
                        <TopItemBar key={i} index={i} label={prov} value={deuda} totalValue={kpis.totalDeuda} color="rojo" />
                    ))}
                </div>
            </div>
        </div>
...
function TabAlertas({ alertasData, kpis, onOpenModal }) {
    const [filterAlerta, setFilterAlerta] = useState('');
    const filteredAlertas = alertasData.filter(a => !filterAlerta || a.nivel === filterAlerta);
    ...
    return (
        <div>
            <div style={{ display: 'flex', gap: 10, alignItems: 'center', marginBottom: 16, flexWrap: 'wrap' }}>
                ...
                <button className="btn btn-secondary btn-sm" onClick={onOpenModal}>⚙️ Umbrales</button>
            </div>
...
function TabDetalle({ filtered, tiposDocUnicos }) {
...
function TabAntiguedad({ filtered }) {
    ...
    const antigData = useMemo(() => {
        ...
        conFecha.forEach(r => {
            ...
            if (d == null) return;
            const b = d <= 30 ? '0-30' : d <= 60 ? '31-60' : d <= 90 ? '61-90' : '91+';
            buckets[b]++;
            bucketsMonto[b] += r.d;
            sumDias += d;
            count++;
        ...
        const mas60 = buckets['61-90'] + buckets['91+'];
...
function TabConcepto({ filtered, kpis }) {
            ...
            <div className="card" style={{ marginBottom: 16 }}>
                ...
                <div className="card-body">
                    {conceptoData.slice(0, 15).map((c, i) => (
                        <TopItemBar key={i} index={i} label={c.cp} value={c.d} totalValue={kpis.totalDeuda} color={CHART_COLORS[i % CHART_COLORS.length]} customScale={5} />
                    ))}
                </div>
            </div>
...
// ═══════════════════════════════════════════════════════════════════════════════
// COMPONENTES DE ESTRUCTURA Y UI REUSABLES
// ═══════════════════════════════════════════════════════════════════════════════
...
function TopBar({ rawDataLength, onOpenModal }) {
    return (
        <div className="topbar">
            ...
            <button className="btn btn-secondary btn-sm no-print" onClick={onOpenModal}>⚙️ Umbrales</button>
            <button className="btn btn-primary btn-sm no-print" onClick={() => window.print()}>🖨️ Exportar PDF</button>
            <div className="topbar-status"><span className="dot" />{rawDataLength.toLocaleString()} registros</div>
        </div>
    );
}

function PageHeader({ filterUE, total, filteredCount }) {
    return (
        <div className="page-header">
            ...
            <div className="page-meta-tags" style={{ display: 'flex', gap: 8, marginTop: 8, flexWrap: 'wrap' }}>
                ...
                <span className="tag tag-naranja">{total.toLocaleString()} registros total</span>
                <span className="tag tag-celeste">{filteredCount.toLocaleString()} con filtros</span>
            </div>
        </div>
    );
}

function FilterZone({ filters, setters, options }) {
    const { filterUE, filterN1, filterDeuda, filterOrigen } = filters;
    const { setFilterUE, setFilterN1, setFilterDeuda, setFilterOrigen } = setters;
            ...
            <div className="filter-row">
                ...
                <div className="filter-group">
                    ...
                    <select className="filter-input" value={filterN1} onChange={e => setFilterN1(e.target.value)}>
                        ...
                        {options.n1Options.map(n => <option key={n} value={n}>{n}</option>)}
                    </select>
                </div>
                <div className="filter-group">
                ...
                <div className="filter-group" style={{ alignSelf: 'flex-end' }}>
                    <button
                        ...
                        onClick={() => { setFilterUE(''); setFilterN1(''); setFilterDeuda('1'); setFilterOrigen(''); }}
                    >
                        ↩ Limpiar
                    </button>
                </div>
...
function TabsBar({ activeTab, setActiveTab, alertasCount }) {
    const tabs = [
        ['kpis', '📊 KPIs y Resumen'],
...
function TopItemBar({ index, label, value, totalValue, color, customScale = 3 }) {
    const pct = totalValue > 0 ? (value / totalValue * 100) : 0;
    const bgColor = color.startsWith('#') ? color : COLORS[color] || color;

    return (
        <div className="progress-row">
            <div className="progress-label" title={label}>{index + 1}. {label}</div>
            <div className="progress-track">
                <div className="progress-fill" style={{ width: `${Math.min(pct * customScale, 100)}%`, background: bgColor }} />
            </div>
            <div className="progress-val">{fmtM(value)}</div>
        </div>
    );
}
