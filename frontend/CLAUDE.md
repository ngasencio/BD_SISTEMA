...
| Formularios FSC | `/abastecimiento/formularios` | ✅ Completo | KPIs + 3 tabs (Solicitudes/Derivados/Productos). Reemplaza el Excel `FSC 2025.xlsx` — datos sincronizados en vivo desde Panel SSO vía Selenium. Botón "Actualizar" pide credenciales (rut/dv/clave) por modal — no hay rango de fechas. Sub-grupo propio en Sidebar, al mismo nivel que "Mercado Público". **Tab "Solicitudes (FSC)" rediseñado (2026-06-08)** con 2 sub-tabs: "Flujo de Visación" (`FlujoVisacion` — pipeline circular P→AC + Rechazados con círculos de tamaño uniforme, drill-down por bandeja vía `/api/formularios/flujo/`) y "Listado de Solicitudes" (`TablaSolicitudes` — buscador server-side, filtro por bandeja, columnas ordenables `SortableTh`, drill-down de productos del carro `ProductosDelFormulario`, ID corto `id_formulario` formato `Fn-XXX-AA`). `TablaDerivados` aplica el mismo patrón de buscador/orden/filtro/drill-down. Tab "Carro de Productos" se mantiene simple (rediseño pendiente — el usuario tiene otra idea para ese tab) |
