...
| Formularios FSC | `/abastecimiento/formularios` | ✅ Completo | Reemplaza el Excel `FSC 2025.xlsx` — datos sincronizados en vivo desde Panel SSO vía Selenium. Botón "Actualizar" pide credenciales (rut/dv/clave) por modal — no hay rango de fechas. Sub-grupo propio en Sidebar, al mismo nivel que "Mercado Público". **Página re-estructurada (2026-06-08, segunda pasada)** siguiendo el layout de `Contratos SSO › Seg. Financiero`: orden **Título → Filtros globales → Tabs → contenido** (sin KPIs sueltos arriba). Filtro global único: selector de **Año**, aplicado vía `?anho=` a stats/flujo/listados/derivados a la vez. 3 tabs principales: Solicitudes (FSC) / Derivados a Comprador / Carro de Productos. El tab **"Solicitudes (FSC)"** usa sub-tabs con el mismo formato visual que `TabFinanciero` (`SubTabBtn`, subrayado violeta): **`📊 Resumen & Gráficos`** (`ResumenFormularios` → KPI grid de 4 tarjetas + `FlujoVisacion` — pipeline circular P→AC + Rechazados con círculos de **tamaño uniforme** (60px) y drill-down por bandeja vía `/api/formularios/flujo/`, ya sin sus propios KPIs/selector de año) y **`📋 Tabla Detalle`** (`TablaSolicitudes` — buscador server-side, **chips de filtro** por bandeja (`FiltroChip`, estilo `TabFinanciero`), columnas ordenables `SortableTh`, drill-down de productos del carro `ProductosDelFormulario`, ID corto `id_formulario` formato `Fn-XXX-AA`). `TablaDerivados` aplica el mismo patrón (buscador/orden/chips de filtro por estado de compra/drill-down), también filtrable por año global. Tooltips "ⓘ" (`InfoTooltip`, mismo patrón que `TabFinanciero`) en KPIs, encabezados de filtro y descripciones del flujo. Tab "Carro de Productos" se mantiene simple (rediseño pendiente — el usuario tiene otra idea para ese tab) |
